...
// y = mx+b
static atomic_t _zeroOffset = 0; // This is b
float _calibrationFactor = 1.0;  // This is m. User provides this number so that
                                 // we can output y when requested

unsigned long _ldoRampDelay =
    250; // During begin, wait this many millis after configuring the LDO before
         // performing calibrateAFE

static bool getRegister(uint8_t registerAddress, uint8_t* value)
{
   bool val;
   k_mutex_lock(&nau_mutex, K_FOREVER);
   val = i2c_write_read(i2c_dev, I2C_ADDR, &registerAddress, 1, value, 1) == 0;
   k_mutex_unlock(&nau_mutex);
   return val;
}
static bool setRegister(uint8_t registerAddress, uint8_t value)
{
   ...
   bool val;
   k_mutex_lock(&nau_mutex, K_FOREVER);
   val = i2c_write(i2c_dev, data, sizeof(data), I2C_ADDR) == 0;
   k_mutex_unlock(&nau_mutex);
   return val;
}
...
bool begin(bool initialize)
{
   ...
   if (initialize)
   {
      ...
      result &=
          setBit(NAU7802_PGA_PWR_PGA_CAP_EN,
                 NAU7802_PGA_PWR); // Enable 330pF decoupling cap on chan 2.
                                   // From 9.14 application circuit note.

      result &= clearBit(
          NAU7802_PGA_LDOMODE,
          NAU7802_PGA); // Ensure LDOMODE bit is clear - improved accuracy and
                        // higher DC gain, with ESR < 1 ohm

      // TODO: remove busy wait
      k_busy_wait(_ldoRampDelay *
                  1000); // Wait for LDO to stabilize - takes about 200ms

      getWeight(true, 10, 1000); // Flush

      result &= calibrateAFE(
          NAU7802_CALMOD_INTERNAL); // Re-cal analog front end when we change
                                    // gain, sample rate, or channel
   }
...
bool setChannel(uint8_t channelNumber)
{
   if (channelNumber == NAU7802_CHANNEL_1)
      return (clearBit(NAU7802_CTRL2_CHS, NAU7802_CTRL2)); // Channel 1
                                                           // (default)
   else
...
{
   int32_t total =
       0; // Readings are 24-bit. We're good to average 255 if needed
...
{
   atomic_set(&_zeroOffset, (atomic_val_t) newZeroOffset);
}

int32_t getZeroOffset()
{
   return (int32_t) atomic_get(&_zeroOffset);
}

// Call after zeroing. Provide the float weight sitting on scale. Units do not
// matter.
void calculateCalibrationFactor(float weightOnScale, uint8_t averageAmount,
                                unsigned long timeout_ms)
{
   int32_t onScale      = getAverage(averageAmount, timeout_ms);
   float newCalFactor = ((float)(onScale - atomic_get(&_zeroOffset))) / weightOnScale;
...
float getWeight(bool allowNegativeWeights, uint8_t samplesToTake,
                unsigned long timeout_ms)
{
   int32_t onScale = getAverage(samplesToTake, timeout_ms);

   int32_t zeroOffset = (int32_t) atomic_get(&_zeroOffset);
   ...
   if (!allowNegativeWeights && onScale < zeroOffset)
   {
      onScale = zeroOffset; // Force reading to zero
   }

   float weight = ((float) (onScale - zeroOffset)) / _calibrationFactor;
   return weight;
}
...
bool get24BitRegister(uint8_t registerAddress, int32_t* value)
{
   ...
   bool val;
   k_mutex_lock(&nau_mutex, K_FOREVER);
   val = i2c_write_read(i2c_dev, I2C_ADDR, &registerAddress, 1, data,
                      sizeof(data));
   k_mutex_unlock(&nau_mutex);
   if (val)
   {
...
bool set24BitRegister(uint8_t registerAddress, int32_t value)
{
   ...
   if (i2c_burst_write(i2c_dev, I2C_ADDR, registerAddress, data,
                       sizeof(data)) != 0)
   {
      return false;
   }
...
{
   uint8_t data[4];
   if (i2c_write_read(i2c_dev, I2C_ADDR, &registerAddress, 1, data,
                      sizeof(data)) != 0)
   {
      return false;
   }
...
{
   uint8_t data[4] = {((value >> 24) & 0xFF), ((value >> 16) & 0xFF),
